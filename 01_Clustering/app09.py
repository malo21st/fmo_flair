...
##v Result Data
result_row = html.Div([
    dbc.Row(id="result-row", no_gutters=True, justify="start"),
...
def set_result_row(cls_data, check_card, check_table, num, arg, trigger, pre_num): # 分類結果ボタンの作成
    changed_id = dash.callback_context.triggered[0]['prop_id']

    num = len(set(cls_data))
    if 'cluster-num' in changed_id:
        result_row = [result_col(i) for i in range(1, num+1)]
        return result_row, dash.no_update
    if 'cluster-arg' in changed_id:
        if pre_num:
            result_row = [result_col(i) for i in range(1, pre_num+1)]
            return result_row, dash.no_update
        else:
            return dash.no_update, dash.no_update
    elif 'cls-data' in changed_id:
        result_row = [result_col(i) for i in range(1, num+1)]
    elif 'check-card' in changed_id:
        if [i[0] for i in check_card if i!=[]]:
            result_row = [result_col(i) for i in range(1, num+2)]
        else:
            result_row = [result_col(i) for i in range(1, num+1)]
    elif 'check-table' in changed_id:
        if [i[0] for i in check_table if i!=[]]:
            result_row = [result_col(i) for i in range(1, num+2)]
        else:
            result_row = [result_col(i) for i in range(1, num+1)]
    else:
...
def toll_data(trigger, check_card, check_table, cls_data, num, select_cls): # 分類結果の集計
    changed_id = dash.callback_context.triggered[0]['prop_id']
    num = len(set(cls_data))
    try:
    ...
    btn_state = [False] * btn_num
    try:
        btn_state[select_cls - 1] = True
    except:
        btn_state = dash.no_update

    return [f"{i}" for i in cls_cnt], btn_disabled, color, btn_state, False, False
...
def classification_list(clicks, num, cls_data, check_card, check_table, selected_cls):
    if dash.callback_context.triggered[0]['value'] == 0:
        raise PreventUpdate

    trigger_id = dash.callback_context.triggered[0]['prop_id'].split('.')[0]

    if 'cluster-num' in trigger_id:
        return dash.no_update, dash.no_update, 0, dash.no_update, dash.no_update

    clicked_id_dic = eval(trigger_id)
