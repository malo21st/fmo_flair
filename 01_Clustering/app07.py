...
def show_mols(clicks, disp_mode, check_item, cls_data, mols_data, item_data, selected_cls):
    # if selected_cls < 1:
    #     return None, [False]*len(clicks), 0
    ...
    if trigger_id not in ['disp-mode', 'set-item']:
        if select_cls:
            btn_state[select_cls - 1] = True

    return mols_cnt, btn_state, select_cls
...
def update(cls_data, trigger, check_card, check_table, num):
    try:
    ...
    return [f"{i}" for i in cls_cnt], btn_disabled
...
def clustering(click, n_close, arg, num, mols_jsn, is_open):
    ...
    elif trigger_id in ['cluster-arg', 'cluster-num']:
        return list(), False, True, True
    else:
        try:
