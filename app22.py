...
IFIE_6graph = html.Div([
    dbc.Modal([
        dbc.ModalHeader(id='id-data-name'),
        dbc.ModalBody([
            dbc.InputGroup([
...
def data_to_graph(param):
    ...
    df_scatter = df_scatter.sort_values('DI SUM')

    X = df_scatter[['DI SUM']].values
    ...
    # fig 1
    trace1_1 = go.Scatter(x=df_scatter['DI SUM'], y=df_scatter['IFIE SUM'], 
        text=df_scatter['NAME'], mode='markers', marker_size=10, showlegend=False)
    trace1_2 = go.Scatter(x=df_scatter['DI SUM'], y=[x[0] for x in lr.predict(X)], 
        mode='lines', line=dict(color='red', width=2), name="OLS", text=["OLS"]*len(X))

    layout1 = go.Layout(
        xaxis = dict(title = 'DI SUM [kcal/mol]'),
...
def toggle_modal(clickData1, clickData2, n2, is_open):
    ...
    if "id-result-2axis" in changed_id:
        return not is_open, clickData1['points'][0]['x'], clickData1['points'][0]['x']
    elif "id-result-scatter" in changed_id:
        name = clickData2['points'][0]['text']
        if name != "OLS":
            return not is_open, name, name
        else:
            return False, "", ""
    elif "close-6graph" in changed_id:
        return not is_open, "", ""
